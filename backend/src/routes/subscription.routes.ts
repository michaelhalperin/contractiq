...
} from "../services/paddle.service.js";
      ...
      const getPlanFromPriceId = (priceId: string): SubscriptionPlan => {
        const priceIds = {
          pro:
            ...
            process.env.PADDLE_SANDBOX_PRICE_ID_PRO ||
            "",
          business:
            process.env.PADDLE_PRICE_ID_BUSINESS ||
            process.env.PADDLE_SANDBOX_PRICE_ID_BUSINESS ||
            "",
          enterprise:
            process.env.PADDLE_PRICE_ID_ENTERPRISE ||
            process.env.PADDLE_SANDBOX_PRICE_ID_ENTERPRISE ||
            "",
        };
      ...
      switch (event.event_type) {
        case "transaction.completed": {
          ...
          if (customerId || customerEmail) {
            ...
            let user = customerId
              ? await User.findOne({ paddleCustomerId: customerId })
              : null;
        ...
        case "subscription.updated": {
          ...
          if (!user) {
            ...
            res.json({
              received: true,
              message: "User not found, will retry later",
            });
            return;
          }
          ...
          if (!subscription) {
            subscription = new Subscription({
              ...
              currentPeriodStart: billingPeriod?.starts_at
                ? new Date(billingPeriod.starts_at)
                : new Date(),
              currentPeriodEnd: billingPeriod?.ends_at
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      // Cancel subscription at period end via Paddle API
      await cancelSubscriptionAtPeriodEnd(user.paddleSubscriptionId);
